...
import { deleteLicense } from '../../services/api';
import { useState } from 'react';
import Alert from '../../../../frontend/src/components/Alert';
import { IconButton, Typography } from '@mui/material';
import { XMarkIcon } from '@heroicons/react/24/solid';

export default function DeleteLicenseModal({ open, handleOpen, licenseId, navigate }) {
  const [alert, setAlert] = useState(null);

  const handleDelete = async () => {
    try {
      await deleteLicense(licenseId);
      setAlert({ message: 'License deleted successfully!', type: 'success' });
      handleOpen(); 
      navigate('/licenses');
    } catch (error) {
      console.error('Error deleting license:', error);
      setAlert({ message: 'Error deleting license. Retry.', type: 'error' });
    }
  };
      ...
      <Dialog open={open} handler={handleOpen} dismissible={false}>
        ...
        <DialogBody className='border-t'>
          <div className='flex flex-col gap-4 py-4'>
            <p className='text-black'>Are you sure you want to delete this license? This action cannot be undone.</p>
          </div>
